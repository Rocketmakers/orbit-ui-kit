...
export const OrbitTabbedView: React.FunctionComponent<ITabbedViewProps> = ({ tabs, activeTab }) => {
  ...
  const createPath = React.useCallback((path: string) => path && (path.startsWith("/") ? `${url}${path}` : `${url}/${path}`), [url]);
  const defaultTab = React.useMemo(() => tabs && tabs.length && tabs[0].to && createPath(tabs[0].to), [tabs, url]);
    ...
    <OrbitView className="orbit-tabbed-view" mode="full-width">
        ...
        {tabs.map((tab, i) => (
          <React.Fragment key={i}>
            {activeTab !== undefined ? (
              ...
              <NavLink className="tab" activeClassName="active" to={createPath(tab.to)}>
                <TabInner {...tab} />
              </NavLink>
            )}
          </React.Fragment>
      ...
      <div className="tabs-inner">
        {activeTab !== undefined ? (
          ...
          <Switch>
            {tabs.map(tab => (
              <Route path={createPath(tab.to)} component={tab.component} />
            ))}
