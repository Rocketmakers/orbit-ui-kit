...
export async function publish() {
  ...
  const currentVersion = await exec(
    "node",
  ...
  console.log(`🛰️ Orbit UI kit version ${currentVersion} [${branch}]`);
  ...
  const response = await prompts([
    {
      ...
      choices: [
        ...
        {
          title: `Pre   [${semver.inc(currentVersion, "prerelease", preId)}]`,
          value: "prerelease"
        }
      ]
    }
  ...
  if (confirmation.confirm) {
    await exec("npm", ["version", releaseType, `--preid=${preId}`], false);
    ...
    if (pushConfirmation.confirm) {
      await exec("git", ["push", "--follow-tags"], false);
    } else {
      console.log(
        "No worries 😎. When you're happy just run 'git push --follow-tags' to start the publishing process."
      );
    }
  }
}
